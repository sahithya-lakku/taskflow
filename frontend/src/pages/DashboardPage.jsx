...
export default function DashboardPage() {
  ...
  return (
    <div>
      <Navbar />
      <main className="mx-auto lg:ml-64 flex w-full max-w-6xl flex-col gap-6 px-4 py-6">
        <section className="glass-card bg-gradient-to-r from-indigo-500/25 via-violet-500/15 to-sky-500/20 p-6">
          <p className="text-sm uppercase tracking-[0.2em] text-indigo-700">Workspace overview</p>
          <h1 className="mt-2 text-3xl font-bold text-slate-900">Dashboard</h1>
          <p className="mt-2 text-slate-700">Build project spaces, collaborate, and track tasks in real time.</p>
          <div className="mt-4 inline-flex rounded-full border border-slate-200 bg-white px-3 py-1 text-sm text-slate-100">{projectCountText}</div>
        </section>
        ...
        {analytics && (
          <section className="grid gap-4 lg:grid-cols-3">
            <div className="glass-card p-4">
              <h3 className="mb-2 text-slate-900">Completion Trend</h3>
            ...
            <div className="glass-card p-4">
              <h3 className="mb-2 text-slate-900">Priority Split</h3>
            ...
            <div className="glass-card p-4">
              <h3 className="mb-2 text-slate-900">Time Spent</h3>
        ...
        <section>
          ...
          {!loading && !error && projects.length === 0 && <div className="glass-card p-4 text-slate-600">No projects yet. Create your first project above.</div>}
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {projects.map((project) => (
                <Link key={project.id} to={`/projects/${project.id}`} className="glass-card group p-4 transition hover:-translate-y-1 hover:border-indigo-300/50">
                  ...
                  <span className="mt-3 inline-block text-xs text-indigo-700">Open board →</span>
                </Link>
              ))}
            </div>
          )}
